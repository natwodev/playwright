...
import { ACTION_TIMEOUT, EXPECT_TIMEOUT, NAVIGATION_TIMEOUT, TEST_TIMEOUT } from 'utils/constants';
import { WaitForLoadStateOptions } from 'globals/optional-parameter-types';
import { defineConfig, devices } from '@playwright/test';
import dotenv from 'dotenv';
dotenv.config({ path: '.env' });

const BASE_URL = process.env.URL || 'https://xspire-test.hqsoft.vn/';
...
export default defineConfig({
   ...
   */
  reporter: [['./src/globals/custom-logger.ts'], ['html', { open: 'never' }], ['dot']],
  /**
   * Shared settings for all the projects below.
   * See https://playwright.dev/docs/api/class-testoptions
   */
  globalSetup: require.resolve('./src/globals/global-setup.ts'),
  globalTeardown: require.resolve('./src/globals/global-teardown.ts'),
